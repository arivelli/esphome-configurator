<template>
  <div class="w-full">
    ...
    <div class="mt-1 relative">
      <date-picker language="es" :input-class="classes" :type="type" :disabled="disabled"
        :time-picker-options="timePickerOptions" :value-type="valueType" v-model:value="dateValue"
        :placeholder="placeholder" @change="updateValue" :format="format" :minute-step="minuteStep"
         @open="$emit('update:error', null)"
        ></date-picker>
...
const updateValue = () => {
  ...
  if (props.change && typeof props.change.value === 'function') {
    props.change.value();
  }
}
